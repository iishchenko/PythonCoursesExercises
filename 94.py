...
def write_movie_info(movie_name, actors):
    my_file = open(movie_name, "a")
    # get the list of dictionary keys (film names), and convert it into a sorted list
    key_list = list(actors.keys())
    key_list.sort()
    # get the values under each key (list of actors in each film)
    for key in key_list:
        line = str(key) + ": "
        # get the list of values for the current key
        values_list = actors[key]
        values_list.sort()
        # iterate over the values list and add to our line each of the actors name
        for item in values_list:
            line += item + ", "
        # finally, let's remove the last coma and whitespace
        line = line[:-2]
        my_file.write(line + "\n")
